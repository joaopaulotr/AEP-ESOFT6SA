...
class TestDecideNode:
    def test_deepen_keeps_action_and_utterance(self, interview_service, fake_llm, state):
        fake_llm.queue(action="deepen", next_utterance="Por que isso te incomoda?")

        result = interview_service._decide_node(graph_state(state))

        assert result["action"] == "deepen"
        assert result["next_utterance"] == "Por que isso te incomoda?"

    def test_preserves_the_untouched_state_keys(self, interview_service, fake_llm, state):
        fake_llm.queue(action="deepen", next_utterance="E depois?")

        result = interview_service._decide_node(graph_state(state, last_answer="trabalho com suporte"))

        assert result["last_answer"] == "trabalho com suporte"
        assert result["interview_state"] is state

    def test_merges_profile_updates_into_the_interview_state(self, interview_service, fake_llm, state):
        fake_llm.queue(profile_updates={"cargo": "analista", "sentimento": "frustrado"})

        interview_service._decide_node(graph_state(state))

        assert state.profile == {"cargo": "analista", "sentimento": "frustrado"}

    def test_profile_updates_overwrite_existing_keys(self, interview_service, fake_llm, state):
    ...
    def test_tolerates_response_without_optional_keys(self, interview_service, fake_llm, state):
    ...
    def test_finish_before_the_last_question_is_downgraded_to_advance(self, interview_service, fake_llm, state):
    ...
    def test_finish_on_the_last_question_uses_the_closing_statement(self, interview_service, fake_llm, state):
    ...
    def test_finish_falls_back_to_the_default_closing(self, interview_service, fake_llm, state):
        state.current_index = LAST_INDEX
    ...
    def test_prompt_carries_the_current_and_remaining_questions(self, interview_service, fake_llm, state):
        state.current_index = 1
        fake_llm.queue()

        interview_service._decide_node(graph_state(state, last_answer="uso todo dia"))
    ...
    def test_prompt_says_nenhuma_when_no_question_remains(self, interview_service, fake_llm, state):
        state.current_index = LAST_INDEX
    ...
    def test_prompt_carries_the_recent_history(self, interview_service, fake_llm, state):
    ...
    def test_unqueued_call_fails_loudly(self, interview_service, state):
        with pytest.raises(AssertionError, match="Chamada inesperada"):
            interview_service._decide_node(graph_state(state))


class TestCompiledGraph:
    def test_compiled_graph_runs_the_decide_node(self, interview_service, fake_llm, state):
        fake_llm.queue(action="deepen", next_utterance="Me dá um exemplo?")

        result = interview_service._graph.invoke(graph_state(state))
...
class TestRunTurn:
    def test_records_the_participant_and_the_interviewer_turns(self, interview_service, fake_llm, state):
        fake_llm.queue(action="deepen", next_utterance="Como assim?")

        interview_service.run_turn(state, "é confuso")
    ...
    def test_returns_the_same_state_object_and_the_utterance(self, interview_service, fake_llm, state):
        fake_llm.queue(action="deepen", next_utterance="Como assim?")

        returned_state, utterance = interview_service.run_turn(state, "é confuso")

        assert returned_state is state
        assert utterance == "Como assim?"

    def test_deepen_does_not_advance_the_interview(self, interview_service, fake_llm, state):
        fake_llm.queue(action="deepen", next_utterance="Pode detalhar?")

        interview_service.run_turn(state, "mais ou menos")

        assert state.current_index == 0
        assert state.asked_indices == []
        assert state.finished is False

    def test_advance_moves_to_the_next_question(self, interview_service, fake_llm, state):
        fake_llm.queue(action="advance", next_utterance="Vamos ao próximo ponto.")

        _, utterance = interview_service.run_turn(state, "cobri tudo")
    ...
    def test_advance_without_utterance_falls_back_to_the_core_question(self, interview_service, fake_llm, state):
    ...
    def test_advance_past_the_last_question_finishes_the_interview(self, interview_service, fake_llm, state):
        state.current_index = LAST_INDEX
    ...
    def test_finish_marks_the_state_as_finished(self, interview_service, fake_llm, state):
        state.current_index = LAST_INDEX
        fake_llm.queue(action="finish", closing_statement="Obrigado, foi ótimo!")

        _, utterance = interview_service.run_turn(state, "acho que é isso")

        assert state.finished is True
        assert utterance == "Obrigado, foi ótimo!"

    def test_profile_accumulates_across_turns(self, interview_service, fake_llm, state):
        ...
        interview_service.run_turn(state, "sou designer")
        interview_service.run_turn(state, "ando cansado")

        assert state.profile == {"cargo": "designer", "sentimento": "cansado"}

    def test_history_sent_to_the_model_grows_with_the_conversation(self, interview_service, fake_llm, state):
        fake_llm.queue(action="deepen", next_utterance="Conta mais.")
        fake_llm.queue(action="deepen", next_utterance="E depois?")

        interview_service.run_turn(state, "primeira resposta")
        interview_service.run_turn(state, "segunda resposta")
    ...
    def test_walks_through_every_core_question_until_finished(self, interview_service, fake_llm, state):
        for _ in CORE_QUESTIONS:
            fake_llm.queue(action="advance", next_utterance="")

        utterances = [interview_service.run_turn(state, "respondi")[1] for _ in CORE_QUESTIONS]
